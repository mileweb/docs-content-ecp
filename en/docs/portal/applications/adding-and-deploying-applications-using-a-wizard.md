...
**Note:** You can add and configure multiple containers and couple them in a pod.

5. At the top right of the wizard, click **\> Next**. 
6. Select whether to expose your application using ECP defined [layer 4 load balancer](<#exposing-via-a-load-balancer>) or the [public network interfaces of a pod replica](<#exposing-via-the-public-network-interfaces-of-a-pod-replica>), and then complete the remaining fields based on your selection.
...
| Backend Selector                                                                                                                                           | Read-only field that shows the pods sitting behind the layer 4 load balancer based on labels. These are the pods you specified in the first wizard step. |
